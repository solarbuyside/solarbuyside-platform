...
**Landing:** a LP **OFICIAL na raiz `/` é a versão "v1"** (`src/AppV1.tsx`, eager). O
redesign **V4 "Solar Dawn" está em PREVIEW em `/v4`** (`apps/landing/src/v4/`, lazy). (Houve
uma janela em 2026-06-11 em que a V4 foi promovida à raiz; revertido a pedido.) O teaser do formulário de lead é o
...
- **Histórico de Atividades** (`/historico`): dados **reais** (`listActivityHistory` — eventos de `comparison_events` + título da avaliação), não mais hardcoded. Timeline com ícone/cor por tipo de evento e estado vazio para conta nova.
- **Manual** (`/manual`, leitor pdf.js de página única): PDF otimizado **62MB → 46MB** (imagens ≤1200px/q80 + subset de fontes); `Cache-Control: immutable` para `*.pdf` no `next.config` (reabrir = instantâneo); Range requests ativos (baixa só a página) + **prefetch** da janela vizinha (`-2..+3`) em background. Para encolher mais (linearizar/<15MB) precisaria de ghostscript (não instalado na máquina).
...
- **2FA por e-mail no login** (anti-compartilhamento): login de NÃO-admin cai em `/verificar`, que tem **botão "Enviar código por e-mail"** (envio explícito — não auto-envia, evita chegar na tela sem código). Após enviar (`?sent=1`): aparece o campo de 6 dígitos + "Entrar" + "Reenviar código" (com **cooldown de 45s**, contagem regressiva no botão). Código de 6 dígitos em `login_2fa_codes` (migration 0016), via Brevo. Cookie `sb-2fa` (HMAC) por login (12h); apagado a cada login/logout. Admins não passam pelo 2FA. Reset de senha / 1º acesso Greenn **não** caem no `/verificar`: ao definir a senha (`updatePasswordAction`), o cookie 2FA é setado (o link do e-mail já provou posse).
...
0b. **(2026-06-09) Confirmar rubricas sinalizadas** com o Francis (prazo de instalação
   ...
   `auto-scoring.ts`.
0c. **(opcional) Promover o redesign V4 da landing** (`apps/landing/src/v4/`, hoje em
   PREVIEW em `/v4`) para a raiz, quando aprovado. (Foi promovido em 2026-06-11 e revertido
   a pedido — segue em /v4 sendo lapidado.)
