...
export type SavedCompanyOption = { id: string; companyName: string };

export function NovaForm({ savedCompanies }: { savedCompanies: SavedCompanyOption[] }) {
  ...
  // Saved-company id backing each slot (same index as competitors), or null.
  const [savedIds, setSavedIds] = React.useState<(string | null)[]>([null, null, null]);
  const [error, setError] = React.useState<string | null>(null);
  const [isPending, startTransition] = React.useTransition();

  const usedSavedIds = new Set(savedIds.filter(Boolean) as string[]);

  const applySavedCompany = (option: SavedCompanyOption) => {
    setCompetitors((prev) => {
      const next = [...prev];
      let slot = next.findIndex((n) => n.trim() === "");
      if (slot === -1) {
        if (next.length >= 6) return prev;
        next.push("");
        slot = next.length - 1;
      }
      next[slot] = option.companyName;
      setSavedIds((ids) => {
        const copy = [...ids];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    competitors.forEach((name, i) => {
      if (name.trim() === "") return;
      formData.append("competitorNames", name.trim());
      const savedId = savedIds[i];
      if (savedId) formData.append("savedCompanyIds", savedId);
    });
            ...
            <form onSubmit={handleSubmit} className="divide-y divide-slate-100">
              ...
              {step === 2 && (
                <div className="p-6 md:p-8 space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-300">
                  ...
                  {/* Saved companies (reuse from library) */}
                  {savedCompanies.length > 0 && (
                    <div className="rounded-lg border border-primary/15 bg-primary/[0.03] p-3.5">
                      <p className="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                        ...
                        Reaproveitar empresa salva
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {savedCompanies.map((sc) => {
                          const used = usedSavedIds.has(sc.id);
                          return (
                            <button
                              key={sc.id}
                              type="button"
                              disabled={used}
                              onClick={() => applySavedCompany(sc)}
                              className={cn(
                                "inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold transition-all",
                                used
                                  ? "cursor-not-allowed border-emerald-500/30 bg-emerald-50 text-emerald-600"
                      ...
                      <p className="mt-2 text-[10px] text-slate-400">
                        Os dados de empresa e técnicos salvos serão pré-preenchidos automaticamente.
                      </p>
                    </div>
                  )}
